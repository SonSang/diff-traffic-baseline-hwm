...
anim_car ogre_car_model::create_car(SceneManager *sm, SceneNode *base, const std::string &name) const
{
    ...
    Entity *lent = ent2;
    for(int i = 0; i < static_cast<int>(lent->getNumSubEntities()); ++i)
    {
    ...
    for(int j = 0; j < static_cast<int>(ent2->getNumManualLodLevels()); ++j)
    {
...
void ogre_car_db::load_meshes()
{
        ...
        ++current)
    {
        ...
        for(int i = 0; i < static_cast<int>(ocm.materials.size()); ++i)
        {
            ...
            ocm.materials[i]->getTechnique(1)->getPass(0)->setDiffuse(ocm.cm->body_colors[i].rgb[0], ocm.cm->body_colors[i].rgb[1], ocm.cm->body_colors[i].rgb[2], 1.0f);
            ocm.materials[i]->getTechnique(1)->getPass(0)->setAmbient(ocm.cm->body_colors[i].rgb[0], ocm.cm->body_colors[i].rgb[1], ocm.cm->body_colors[i].rgb[2]);
